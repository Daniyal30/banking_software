                        ...
                        <tbody>
                            ...
                            @forelse ($loans as $loan)
                                <tr>
                                    ...
                                    <td class="fw-semibold">{{ $loan->lender->name }}</td>
                                    <td>{{ number_format($loan->amount, 0) }}</td>
                                    <td>{{ \Carbon\Carbon::parse($loan->loanDate)->format('d-m-Y') }}</td>
                                    <td class="text-center">
                                        {{-- <a href="{{ route('loan.show', $loan->id) }}" class="btn btn-sm btn-outline-info" title="View">
                                            <i class="fas fa-eye"></i>
                                        </a> --}}
                                        <a href="{{ route('loan.edit', $loan->id) }}" class="btn btn-sm btn-outline-warning" title="Edit">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <button type="button" class="btn btn-sm btn-outline-danger delete-btn" title="Delete"
                                            data-id="{{ $loan->id }}" data-url="{{ route('loan.destroy', $loan->id) }}">
